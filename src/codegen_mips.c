...
static Temp_tempList munchArgs(T_expList args) {
	Temp_tempList res = NULL, last = NULL;
	T_expList argList = args;
	for (argList; argList; argList = argList->tail) {
		if (last == NULL) {
			res = last = Temp_TempList(munchExp(argList->head), NULL);
		}
		else {
			last = last->tail = Temp_TempList(munchExp(argList->head), NULL);
		}
	}
	return res;
}
